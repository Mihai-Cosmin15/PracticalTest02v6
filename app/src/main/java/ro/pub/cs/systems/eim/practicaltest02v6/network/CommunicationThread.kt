...
//                    val request = Request.Builder().url("").post(formBody).build()
                    ...
                    // DE SCHIMBAT POSIBIL
                    val value = content.getJSONObject("DATA").getJSONObject("BTC-${coin}").getJSONObject("VALUE")

                    val info = Information(value = value.toString(), updated = "acum")

                    // DE SCHIMBAT
                    serverThread.setData(value.toString(), info)
